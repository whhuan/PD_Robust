...
Files in `docs/`, including `docs/reference/`, are generated website output,
ignored by Git, and must not be committed to `main` or maintained manually.
The Action generates `docs/` and deploys its contents to the root of `gh-pages`,
which is the GitHub Pages publishing source. Local website builds are for
preview only; publish website changes through the Action.
Vignette sources remain in `vignettes/`, and website configuration remains in
`_pkgdown.yml`.

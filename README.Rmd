...
The methodological reference is Zhang et al. (2026),
[A Novel Tool for Evaluating Effect Modification in Older Adults with ADRD
Using Medicare Claims](https://doi.org/10.48550/arXiv.2608.06654).
Use `citation("PDRobust")` for the software and paper citations.

**Treatment coding matters.** This release retains the 0.3.7 convention:
treatment `1` is the survival-favorable arm. The paper's main estimator uses
the opposite labels. Recode paper-coded treatment as `1 - A` before mapping
and standardizing, then negate estimates and swap/negate confidence limits
to report the paper's contrast. The bundled examples already use the package
convention. Read `vignette("method-and-coding", package = "PDRobust")` before
adapting the workflow to a study. Data checks cannot verify the causal
identifying assumptions, and `SA()` provides outcome-noise sensitivity rather
than the paper's principal-ignorability sensitivity analysis.
...
`FALSE`. Use `drop = TRUE` only when the reported subject-level exclusions
are intended.
...
propensity-score model, `prin_fo` the principal-score model, and `out_fo`
the outcome model.

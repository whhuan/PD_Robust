...
[`DataCheck()`](https://whhuan.github.io/PD_Robust/reference/DataCheck.md)
returns a validation report without modifying the input dataset. When
`strict = TRUE`, the function raises an error if one or more failed
checks are marked as analysis-blocking. Missing required columns or
empty data cause an early return containing only the checks that can be
...
| `ready_for_analysis` | `TRUE` when no failed check is marked as analysis-blocking. Raw data still need [`DataStandard()`](https://whhuan.github.io/PD_Robust/reference/DataStandard.md) to attach the mapping and readiness metadata required by the HTE and diagnostic interfaces. |

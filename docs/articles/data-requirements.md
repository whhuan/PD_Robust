...
which is the default, `DataCheck()` returns the complete validation
report without modifying the input dataset. When `strict = TRUE`, the
function raises an error if one or more failed checks are marked as
analysis-blocking.

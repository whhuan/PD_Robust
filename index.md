...
defaults to `FALSE`. Use `drop = TRUE` only when the reported
...
estimates time-specific heterogeneous treatment effects. For each time
in `target_time`, it returns the intercept and coefficients of the
mapped effect modifiers, together with a forest plot.
